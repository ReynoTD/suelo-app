...
const CustomInput = ({ label, value, onChangeText, multiline = false, keyboardType = 'default' }) => (
...
const ReporteBasicoForm = () => {
  ...
  const handleGuardarReporte = () => {
    ...
    const fechaReporteISO = isNaN(dateToSave.getTime()) ? 'Error al parsear fecha o formato incorrecto' : dateToSave.toISOString();
              ...
              <View style={styles.centeredView}>
                <View style={styles.modalView}>
                  ...
                  {RIESGOS.map((risk, index) => (
                    <Pressable
                      key={index}
                      style={[styles.modalOption, index === RIESGOS.length - 1 && styles.lastModalOption]}
                      onPress={() => {
